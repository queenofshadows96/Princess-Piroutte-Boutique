...
export async function POST(req: Request) {
  try {
    ...
    if (itemsError) console.error("Shipped items fetch error:", itemsError);
    ...
    try {
      ...
      console.error("Shipping address parse error (shipped email):", e);
    }

    // USPS estimated delivery (3–5 business days)
    let estimatedDelivery = "";
    if (shipment.shipped_at) {
      const shippedDate = new Date(shipment.shipped_at);
      const estimate = new Date(shippedDate);
      estimate.setDate(shippedDate.getDate() + 4); // midpoint of 3–5 days
      estimatedDelivery = estimate.toLocaleDateString("en-US", {
        weekday: "long",
        month: "long",
        day: "numeric",
      });
            ...
            <table width="600" cellpadding="0" cellspacing="0" border="0"
              ...
              <tr><td style="padding:30px;font-family:'Times New Roman',serif;color:#444;font-size:17px;line-height:1.7;">
                <h1 style="font-family:'Great Vibes',cursive;font-size:42px;color:#D4AF37;text-align:center;margin:0 0 20px;">
                  Your Royal Order Has Shipped ✨
                </h1>
                <p>Hello Princess! 👸🏽 Your royal treasures are on their way!</p>
                <p><strong>Carrier:</strong> USPS<br/>
                ...
                <div style="text-align:center;margin:30px 0;">
                  <a href="${process.env.NEXT_PUBLIC_SITE_URL}/shipping/track/${trackingToken}"
